    ...
    for (int i = 0; i < fstab->num_entries; i++) {
        ...
        if (path_type == PathTypes::kMountPoint) {
            ...
            if (std::string(fstab->recs[i].fs_type) == "f2fs" &&
                ...
                std::string pathToPush = std::string("/sys/fs/") + fstab->recs[i].fs_type +
                                 "/" + Basename(gc_path);
                if(access(pathToPush.c_str(), F_OK)) {
                    std::string block_device = BlockDeviceForPath(std::string(fstab->recs[i].mount_point) + "/");
                    if (!block_device.empty()) {
                        pathToPush = std::string("/sys/fs/") + fstab->recs[i].fs_type +
                                 "/" + Basename(block_device);
                    }
                }
...
static int stopGc(const std::list<std::string>& paths) {
    for (const auto& path : paths) {
        ...
        //prize modify by liuwei, optimize f2fs discard size, 2018-12-18 begin
        //if (!WriteStringToFile("16", path + "/discard_granularity")) {
        if (!WriteStringToFile("100", path + "/discard_granularity")) {
        //prize modify by liuwei, optimize f2fs discard size, 2018-12-18 end
            PLOG(WARNING) << "Set discard gralunarity failed on" << path;
        }
    }
    return android::OK;
