...
static int show_resmon_stat(struct seq_file *m, void *v)
{
	...
	for_each_process(task) {
		...
		if (lock_task_sighand(task, &flags)) {
			...
			if (!strstr(secctx, RESMON_FILTER))
			{
				unlock_task_sighand(task, &flags);
				continue;
			}
			...
			seq_put_decimal_ull(m, ' ', (long long unsigned int)cputime_to_clock_t(utime));
			seq_put_decimal_ull(m, ' ', (long long unsigned int)cputime_to_clock_t(stime));
			...
			pos += sprintf(test + pos, "%llu ", (long long unsigned int)cputime_to_clock_t(utime) );
			pos += sprintf(test + pos, "%llu ", (long long unsigned int)cputime_to_clock_t(stime) );
