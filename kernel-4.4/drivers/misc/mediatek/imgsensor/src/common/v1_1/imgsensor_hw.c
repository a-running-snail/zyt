...
//prize  modify by zhuzhengjiang  for mipi switch 20190121-begin
		...
		enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	PK_DBG("sensor_idx %d, power %d curr_sensor_name %s\n", sensor_idx, pwr_status,
	       curr_sensor_name);
    //prize  modify by zhuzhengjiang  for ATA camera main:gc8034,sub:gc8044  20190121-begin
    if((sensor_idx ==IMGSENSOR_SENSOR_IDX_MAIN  && !strcmp("gc8044_mipi_raw", curr_sensor_name)) ||
	    (sensor_idx ==IMGSENSOR_SENSOR_IDX_SUB  &&  !strcmp("gc8034_mipi_raw", curr_sensor_name)))
    {
        return IMGSENSOR_RETURN_ERROR;
    }
    //prize  modify by zhuzhengjiang  for ATA camera main:gc8034,sub:gc8044  20190121-end
     //prize  modify by zhuzhengjiang  for mipi switch 20190121-begin
     if(pwr_status == IMGSENSOR_HW_POWER_STATUS_ON)
	{
	...
	else if(pwr_status == IMGSENSOR_HW_POWER_STATUS_OFF && sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
	{
     ...
     //prize  modify by zhuzhengjiang  for mipi switch 20190121-end
