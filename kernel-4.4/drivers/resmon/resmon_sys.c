...
static void resmon_process(void)
{
	...
	int cur_loads, cur_iowait, cur_mem = 0;
	...
	if (resmon_obj->cpuloads != 0)
	{
		...
		resmon_debug("%s cur_loads:%d cur_iowait:%d cur_mem:%d\n", __func__, cur_loads, cur_iowait, cur_mem);
...
static int _resmon_timer_callback(unsigned long data)
{
                                ...
                                char *buf)
{
	...
	for(i = CPUTIME_USER; i < NR_STATS; i++)
		pos += snprintf(buf + pos, 64, " %llu", cputime64_to_clock_t(cpustat[i]));
