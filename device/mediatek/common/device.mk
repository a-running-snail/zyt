...
#  ifeq ($(MTK_SIGNATURE_CUSTOMIZATION),yes)
    #prize-peisaisai-move-20180827-begin
    ...
    #endif
    ifeq ($(wildcard device/mediatek/common/security),)
      $(error Please create device/mediatek/common/security/ and put your releasekey there!!)
    else
      PRODUCT_DEFAULT_DEV_CERTIFICATE := device/mediatek/common/security/releasekey
    endif
    #prize-peisaisai-move-20180827-end
#  else
    #prize-peisaisai-move-20180827-begin
    #prize-signature customization-YZHD-20180110-start
#    $(error MTK_SIGNATURE_CUSTOMIZATION must be yes!!)
...
#prize-peisaisai-move-20180827-begin
#prize-current gms-luoyanzi-20170921-start
ifeq ($(strip $(PRIZE_CURRENT_GMS)),)
$(error there is an error,PRIZE_CURRENT_GMS is not set!)
endif
#prize-gms eea-YZHD-20181217-begin
ifeq ($(strip $(BUILD_AGO_GMS)), yes)
  ifeq ($(strip $(PRIZE_GMS_SEND_TYPE_EEA)),type4c)
$(call inherit-product-if-exists, gms/$(strip $(PRIZE_CURRENT_GMS))/products/gms_go_eea_type4c.mk)
  else
$(call inherit-product-if-exists, gms/$(strip $(PRIZE_CURRENT_GMS))/products/gms_go.mk)
  endif
else
  ifeq ($(strip $(PRIZE_GMS_SEND_TYPE_EEA)),type4c)
$(call inherit-product-if-exists, gms/$(strip $(PRIZE_CURRENT_GMS))/products/gms_eea_type4c.mk)
  else
$(call inherit-product-if-exists, gms/$(strip $(PRIZE_CURRENT_GMS))/products/gms.mk)
  endif
endif
#prize-gms eea-YZHD-20181217-end
#prize-current gms-luoyanzi-20170921-end

#prize-cts-luoyanzi-20180125start
ifeq ($(strip $(PRIZE_IS_CARRIERS)), yes)
    PRODUCT_PROPERTY_OVERRIDES += \
      ro.com.google.clientidbase=$(PRIZE_CLIENTIDBASE) \
      ro.com.google.clientidbase.am=$(PRIZE_CLIENTIDBASE)
else
    PRODUCT_PROPERTY_OVERRIDES += \
      	ro.com.google.clientidbase=$(PRIZE_CLIENTIDBASE)	
endif
#prize-cts-luoyanzi-20180125start-end
...
#modify by guofan for  remove system app 20181218  start 
...
#prize-close Baidu_Location-lanweiming-20181127-begin
ifeq ($(BAIDU_LOCATION_APP), yes)
   PRODUCT_PACKAGES += Baidu_Location
   PRODUCT_PACKAGES += liblocSDK6c
   PRODUCT_PACKAGES += libnetworklocation
endif
#prize-close Baidu_Location-lanweiming-20181127-end
...
ifeq ($(strip $(MICROTRUST_TAMANAGER_SUPPORT)), yes)
  PRODUCT_PACKAGES += UtTAManager
  PRODUCT_PACKAGES += lib_ut_ta_manager
  PRODUCT_BOOT_JARS += lib_ut_ta_manager
endif
...
ifeq ($(strip $(MICROTRUST_FINGERPRINT_SUPPORT)), yes)
  # Prize-wyq-20190116-fix build error of dulplicate define name
  # DEVICE_MANIFEST_FILE += vendor/mediatek/proprietary/trustzone/microtrust/source/common/manifest.microtrust_fp.xml
endif

MICROTRUST_SRC_DIR := vendor/mediatek/proprietary/trustzone/microtrust/source/common/280/teei
TARGET_COPY_OUT_WECHAT := vendor/thh/ta/d78d338b1ac349e09f65f4efe179739d.ta
TARGET_COPY_OUT_ALIPAY_TA :=vendor/thh/ta/08010203000000000000000000000000.ta
  ...
  ifeq ($(strip $(MICROTRUST_WECHAT_SUPPORT)), yes)
    ...
    DEVICE_MANIFEST_FILE += vendor/mediatek/proprietary/trustzone/microtrust/source/common/280/manifest_wechat.xml
    PRODUCT_COPY_FILES += $(call add-to-product-copy-files-if-exists, $(MICROTRUST_SRC_DIR)/wechat:$(TARGET_COPY_OUT_WECHAT))
  endif


  ifeq ($(strip $(MICROTRUST_IFAA_SUPPORT)), yes)
    PRODUCT_BOOT_JARS += ifaa_fw
    PRODUCT_PACKAGES += libteeclientjni libmtee ifaa_service libifaa_jni ifaa_fw libifaa_hal libmtee_hs
    PRODUCT_PACKAGES += vendor.microtrust.hardware.ifaa@1.0-service
    DEVICE_MANIFEST_FILE += vendor/mediatek/proprietary/trustzone/microtrust/source/common/280/manifest_ifaa.xml
    PRODUCT_COPY_FILES += $(call add-to-product-copy-files-if-exists,$(MICROTRUST_SRC_DIR)/alipayapp:$(TARGET_COPY_OUT_ALIPAY_TA))
  endif
...
ifeq ($(MTK_AURISYS_FRAMEWORK_SUPPORT),yes)
  ...
  #M:Prize-chenteng-20180806-Add AudioParamVersion Info File
  PRODUCT_COPY_FILES += device/mediatek/common/audio_param/AudioParamVersionInfo.txt:$(TARGET_COPY_OUT_VENDOR)/etc/audio_param/AudioParamVersionInfo.txt
  #M:chenteng-end
...
# Add Identity
# prize-pss del for gms 20181215-begin
...
# prize-pss del for gms 20181215-end
...
#prize-add preBuildApps-lanwm-20180830-start
$(call inherit-product-if-exists, packages/apps/PreBuildApps/public/apps.mk)
$(call inherit-product-if-exists, packages/apps/PreBuildApps/project/apps.mk)
$(call inherit-product-if-exists, packages/apps/PreBuildApps/3rd-party/apps.mk)
#prize-add preBuildApps-lanwm-20180830-end

#PRIZE-BUILD-huarui-20180112-start
$(call inherit-product-if-exists, vendor/prize/PrizeDriver.mk)
#PRIZE-BUILD-huarui-20180112-end

#prize add by zengke 20180821  ---start
PRODUCT_PACKAGES += PriFactoryTest
ifeq ($(strip $(PRIZE_64B_PROJECT)),yes)
    PRODUCT_PACKAGES += memtester
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_64B_project=1
endif
PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_test=1
#ifeq ($(strip $(PRIZE_HOR_CALI)),yes)
    PRODUCT_PACKAGES += HorCali
    PRODUCT_PACKAGES += libprizeem_sensor_jni
    PRODUCT_PROPERTY_OVERRIDES += ro.hor_cali=1
#endif
ifeq ($(strip $(PRIZE_FACTORY_TEST_REAR_CAMERA_SUB)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_rear_camera_sub=1
endif
ifeq ($(strip $(PRIZE_FACTORY_TEST_REAR_CAMERA_SUB_ALS)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_rear_camera_sub_als=1
endif
ifeq ($(strip $(PRIZE_FACTORY_TEST_REAR_CAMERA_SUB2)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_rear_camera_sub2=1
endif
ifeq ($(strip $(PRIZE_FACTORY_TEST_REAR_CAMERA_SUB3)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_rear_camera_sub3=1
endif
ifeq ($(strip $(PRIZE_FACTORY_TEST_REAR_CAMERA_FRONT_SUB2)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_front_camera_sub2=1
endif
ifeq ($(strip $(PRIZE_LED)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_led=1
endif
ifeq ($(strip $(PRIZE_LED_YC)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_led_yc=1
endif
ifeq ($(strip $(PRIZE_TORCH_FRONT)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.torch_front=1
endif
ifeq ($(strip $(PRIZE_HALL)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_hall=1
endif
ifeq ($(strip $(PRIZE_OTG_TEST)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_otg=1
endif
ifeq ($(strip $(PRIZE_DUAL_MIC_SUPPORT)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.mtk_dual_mic_support=1
endif
ifeq ($(strip $(PRIZE_WIRELESS_CHARGER)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.pri_wireless_charger=1
endif
ifeq ($(strip $(PRIZE_FACTORY_CALIBRATION_SHOW_01)),yes)
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_calibration01=1
else
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_calibration01=0
endif
ifneq ($(strip $(PRIZE_DEFAULT_VALUE_WIRELESS_CHARGER)),)
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_default_value_wireless_charger=$(strip $(PRIZE_DEFAULT_VALUE_WIRELESS_CHARGER))
endif
ifeq ($(strip $(PRIZE_FACTORY_TEST_WBG)),yes)
        PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_test_wbg =1
else
        PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_test_wbg =0
endif
ifeq ($(strip $(PRIZE_FACTORYTEST_DEFAULT_LANGUAGE_EN)),yes)
        PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_default_lang_en =1
else
        PRODUCT_PROPERTY_OVERRIDES += ro.pri_factory_default_lang_en =0
endif
ifneq ($(strip $(PRIZE_CUSTOMER)),)
        PRODUCT_PROPERTY_OVERRIDES += ro.pri_customer=$(strip $(PRIZE_CUSTOMER))
endif
ifeq ($(strip $(PRIZE_DEL_LAN_SWITCH_BTN)),yes)
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_del_lan_switch_btn=1
else
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_del_lan_switch_btn=0
endif
ifneq ($(strip $(PRIZE_BOARD_NETWORK_TYPE)),)
PRODUCT_PROPERTY_OVERRIDES += ro.pri_board_network_type=$(strip $(PRIZE_BOARD_NETWORK_TYPE))
endif
ifneq ($(strip $(PRIZE_TEE)),)
PRODUCT_PROPERTY_OVERRIDES += ro.pri_tee=$(PRIZE_TEE)
endif
ifneq ($(strip $(PRIZE_FINGERPRINT_MODULE)),)
PRODUCT_PROPERTY_OVERRIDES += ro.pri_fingerprint_module=$(PRIZE_FINGERPRINT_MODULE)
endif
ifneq ($(strip $(PRIZE_FINGER_PRINT)),)
PRODUCT_PROPERTY_OVERRIDES += ro.pri.fingerprint = $(PRIZE_FINGER_PRINT)
endif
ifeq ($(strip $(PRIZE_REAL_IC_STEP_SENSOR)),yes)
    PRODUCT_PROPERTY_OVERRIDES += ro.pri.real_ic_step_sensor =1
endif
ifeq ($(strip $(PRIZE_AUTOTEST_APP)),yes)
    PRODUCT_PACKAGES += PrizeAutoTest
    PRODUCT_PROPERTY_OVERRIDES += ro.pri_auto_test=1
    PRODUCT_PACKAGES += autotest_client
    PRODUCT_PACKAGES += HorCali
    PRODUCT_PACKAGES += libprizeem_sensor_jni
    PRODUCT_PROPERTY_OVERRIDES += ro.hor_cali=1
endif
ifeq ($(strip $(PRIZE_BANDMODE_APP)),yes)
    PRODUCT_PACKAGES += PrizeBandMode
endif
ifneq ($(strip $(PRIZE_FUNCTION_KEY)),)
#mute key
ifeq ($(findstring MUTE,$(PRIZE_FUNCTION_KEY)),MUTE)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_mute_function_key=1
endif
#customize key
ifeq ($(findstring CUSTOM,$(PRIZE_FUNCTION_KEY)),CUSTOM)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_custom_function_key=1
endif
#sos key
ifeq ($(findstring SOS,$(PRIZE_FUNCTION_KEY)),SOS)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_sos_function_key=1
	  PRODUCT_PACKAGES += Prizesos	 
endif
#PTT record key
ifeq ($(findstring PTT_RECORD,$(PRIZE_FUNCTION_KEY)),PTT_RECORD)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_ptt_rec_function_key=1
endif
#CAMERA
ifeq ($(findstring CAMERA,$(PRIZE_FUNCTION_KEY)),CAMERA)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_camera_function_key=1
endif
#POWER
ifeq ($(findstring POWER,$(PRIZE_FUNCTION_KEY)),POWER)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_power_function_key=1
endif
#HOME
ifeq ($(findstring HOME,$(PRIZE_FUNCTION_KEY)),HOME)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_home_function_key=1
endif
#BACK
ifeq ($(findstring BACK,$(PRIZE_FUNCTION_KEY)),BACK)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_back_function_key=1
endif
#MENU
ifeq ($(findstring MENU,$(PRIZE_FUNCTION_KEY)),MENU)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_menu_function_key=1
endif
#ONEKEY
ifeq ($(findstring ONEKEY,$(PRIZE_FUNCTION_KEY)),ONEKEY)
     PRODUCT_PROPERTY_OVERRIDES += ro.pri_onekey_function_key=1
endif
endif
#prize add by zengke 20180821  ---end

# begin, prize-lifenfen-20181206, add for new touchscreen goodix_gtx8
ifneq ($(strip $(PRIZE_TOUCH_GOODIX_GT9886)),)
tp_src_files := $(shell ls vendor/tp/goodix)
PRODUCT_COPY_FILES += $(foreach file, $(tp_src_files), \
        vendor/tp/goodix/$(file):vendor/firmware/$(file))
# PRODUCT_COPY_FILES += vendor/tp/goodix/goodix_cfg_group.bin:system/etc/firmware/goodix_cfg_group.bin
endif
...
ifeq ($(findstring smartpa_awinic_aw8898, $(MTK_AUDIO_SPEAKER_PATH)), smartpa_awinic_aw8898)
PRODUCT_COPY_FILES += device/mediatek/common/audio_param_smartpa/aw8898_cfg.bin:vendor/firmware/aw8898_cfg.bin
ifeq ($(strip $(PRIZE_AUDIO_AWINIC_MEC_SKT)), yes)
# prize added by lifenfen, for awinic mec algorithm, 20190215 begin
PRODUCT_COPY_FILES += vendor/mediatek/proprietary/hardware/smartpa/awinic/awinic_params.bin:/vendor/firmware/awinic_params.bin
PRODUCT_COPY_FILES += vendor/mediatek/proprietary/hardware/smartpa/awinic/awinic.audio.effect.so:/vendor/lib/hw/awinic.audio.effect.so
# prize added by lifenfen, for awinic mec algorithm, 20190215 end
# prize added by lifenfen, for optimspeaker arkamys audio solution, 20190219 begin
$(call inherit-product-if-exists, device/mediatek/common/optimspeaker/device-partial.mk)
# prize added by lifenfen, for optimspeaker arkamys audio solution, 20190219 end
endif
endif
...
#prize-add for driveroverlay -by-peisaisai-20181226-begin
DEVICE_PACKAGE_OVERLAYS += driveroverlay/
#prize-add for driveroverlay -by-peisaisai-20181226-end

#prize-add for driveroverlay -by-peisaisai-20180109-begin
DEVICE_PACKAGE_OVERLAYS += publicoverlay/
#prize-add for driveroverlay -by-peisaisai-20180109-end



#prize-add for arcsoft calibration by zhangguo 20190318 begin
ifeq ($(ARCSOFT_DUALCAMERA),yes)
PRODUCT_PACKAGES += ArcSoftCalibration
PRODUCT_COPY_FILES += \
            vendor/mediatek/proprietary/packages/apps/ArcSoftCalibration/jni_calibration/libs/arm64/libmpbase.so:system/lib64/libmpbase.so
endif
#prize-add for arcsoft calibration by zhangguo 20190318 end
ifeq ($(strip $(PRIZE_CUSTOMER_ZYT_CT_VOLTE)), yes)
  PRODUCT_PROPERTY_OVERRIDES += persist.vendor.volte_support=1
  PRODUCT_PROPERTY_OVERRIDES += persist.vendor.mtk.volte.enable=1
  PRODUCT_PROPERTY_OVERRIDES += persist.vendor.mtk_ct_volte_support=1
endif
ifeq ($(strip $(PRIZE_CUSTOMER_ZYT)), yes)
    ### zyt.david add <2019.03.12> begin
	PRODUCT_PROPERTY_OVERRIDES += ro.use.zytsafe=true
	...
	PRODUCT_COPY_FILES += vendor/zyt/lib/libveb_crypto.so:vendor/lib/libveb_crypto.so
	PRODUCT_COPY_FILES += vendor/zyt/lib/libvebhw.so:vendor/lib/libvebhw.so
	PRODUCT_COPY_FILES += vendor/zyt/lib/libVEBSKF_FRI.so:vendor/lib/libVEBSKF_FRI.so
	PRODUCT_COPY_FILES += vendor/zyt/lib/libVEBSKF_STD.so:vendor/lib/libVEBSKF_STD.so
	#PRODUCT_COPY_FILES += vendor/zyt/lib/VEBSKF_ForFRI.so:vendor/lib/VEBSKF_ForFRI.so
	#PRODUCT_COPY_FILES += vendor/zyt/lib/vendor.qti.zyitong.device@1.0.so:vendor/lib/vendor.qti.zyitong.device@1.0.so
	#PRODUCT_COPY_FILES += vendor/zyt/lib/vendor.qti.zyitong.device@1.0_vendor.so:vendor/lib/vendor.qti.zyitong.device@1.0_vendor.so
	PRODUCT_COPY_FILES += vendor/zyt/lib64/libveb_crypto.so:vendor/lib64/libveb_crypto.so
	PRODUCT_COPY_FILES += vendor/zyt/lib64/libvebhw.so:vendor/lib64/libvebhw.so
	PRODUCT_COPY_FILES += vendor/zyt/lib64/libVEBSKF_FRI.so:vendor/lib64/libVEBSKF_FRI.so
	PRODUCT_COPY_FILES += vendor/zyt/lib64/libVEBSKF_STD.so:vendor/lib64/libVEBSKF_STD.so
	#PRODUCT_COPY_FILES += vendor/zyt/lib64/VEBSKF_ForFRI.so:vendor/lib64/VEBSKF_ForFRI.so
	#PRODUCT_COPY_FILES += vendor/zyt/lib64/vendor.qti.zyitong.device@1.0.so:vendor/lib64/vendor.qti.zyitong.device@1.0.so
	#PRODUCT_COPY_FILES += vendor/zyt/lib64/vendor.qti.zyitong.device@1.0_vendor.so:vendor/lib64/vendor.qti.zyitong.device@1.0_vendor.so
	#PRODUCT_COPY_FILES += vendor/zyt/bin/hw/vendor.qti.zyitong.device@1.0-service:vendor/bin/hw/vendor.qti.zyitong.device@1.0-service
	PRODUCT_COPY_FILES += vendor/zyt/hw/lib/veb_skf.default.so:vendor/lib/hw/veb_skf.default.so
	PRODUCT_COPY_FILES += vendor/zyt/hw/lib/veb_skf_fri.default.so:vendor/lib/hw/veb_skf_fri.default.so
	PRODUCT_COPY_FILES += vendor/zyt/hw/lib64/veb_skf.default.so:vendor/lib64/hw/veb_skf.default.so
	PRODUCT_COPY_FILES += vendor/zyt/hw/lib64/veb_skf_fri.default.so:vendor/lib64/hw/veb_skf_fri.default.so
	PRODUCT_COPY_FILES += vendor/zyt/a5SpiUtil_product:data/a5SpiUtil_product
	### zyt.david add <2019.03.12> end
endif
