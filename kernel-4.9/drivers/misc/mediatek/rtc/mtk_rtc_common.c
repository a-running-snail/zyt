...
{
	unsigned long ddr_sign,new_spare2;

	printk("lsw_ddr_sign %s %d\n)",__func__,__LINE__);
	if (kstrtoul(buf, 10, &ddr_sign)) {
		return -EINVAL;
	}

	printk("lsw_ddr_sign %s %d ddr_sign=%lx\n)",__func__,__LINE__,ddr_sign);
	if (ddr_sign < 0) {
		return -EINVAL;
	}
	...
	rtc_write(RTC_AL_DOW, ddr_sign | new_spare2);
	rtc_write_trigger();
	printk("lsw_ddr_sign %s %d ddr_sign=%lx new_spare2=%lx\n)",__func__,__LINE__,ddr_sign,new_spare2);
