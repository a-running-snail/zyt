...
int test_mmu_ddr(unsigned int start_address, unsigned int test_length, unsigned int test_step)
{
    ...
    for(; tested_length < test_length; )
    {
        ...
        {
            video_printf("ddr test ok!! test(0x%x)\n", start_address + tested_length + test_step);
        }
        else
        {
...
#ifdef PRIZE_DDR_TEST
    	U16 now_yea, new_spare2, ddr_sign;
    new_spare2 = RTC_Read(RTC_AL_DOW);
    ddr_sign = new_spare2 >> 8;
    now_yea = RTC_Read(RTC_TC_YEA) + RTC_MIN_YEAR;

    dprintf(CRITICAL, "lsw_rtc_ddr [pmic_init] new_spare2=%d now_yea=%d ddr_sign=%d LK Start..................\n",new_spare2,now_yea,ddr_sign); 
    
	if((boot_reason !=4) && (ddr_sign != 255) && (g_boot_mode == RECOVERY_BOOT))
    {
        mtk_wdt_disable(); //Disable wdt before jump to DA
        ...
        {
		   test_mmu_ddr(0x40000000, 0x7880000, 0x20000);
           //test_mmu_ddr(0x47900000, 0x380000, 0x20000);
		  // test_mmu_ddr(0x47fd0000, 0x30000, 0x20000);
		   test_mmu_ddr(0x48400000, 0x27000000, 0x20000);
         
		   test_mmu_ddr(0x80000000, 0xf000000, 0x20000);
		  
		  
		  
		  
		   test_mmu_ddr(0x8f300000, 0x100f0000, 0x20000);
		   test_mmu_ddr(0x9ff00000, 0x1E100000, 0x20000);
		   test_mmu_ddr(0xbe300000, 0xd00000, 0x20000);
		   test_mmu_ddr(0xbf400000, 0xb40000, 0x20000);
		   test_mmu_ddr(0xC0000000, 0x80000000, 0x20000);
        }
