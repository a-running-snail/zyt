...
UINT32 DoCamCal2AGain_imx499(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
UINT32 DoCamCalSingleLsc_imx499(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
/*zhengjiang.zhu@prize.Camera.Driver  2018/12/15 end for imx499 otp*/
//prize fengshangdong add at 20190117 for imx499 pdaf 
UINT32 DoCamCalPDAF_imx499(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize,UINT32* pGetSensorCalData);

//prize fengshangdong add for 5e9 otp
UINT32 DoCamCal2AGain_5e9(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
UINT32 DoCamCalSingleLsc_5e9(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
//prize end
//prize-camera  add for imx476 OTP  by zhuzhengjiang 20190418-begin
UINT32 DoCamCal2AGain_imx476(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
UINT32 DoCamCalSingleLsc_imx476(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
//prize-camera  add for imx476 OTP  by zhuzhengjiang 20190418-end
...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
     ...
     {//CALIBRATION_LAYOUT_SENSOR_OTP
         0x0000000B, 0x00000001, CAM_CAL_SINGLE_EEPROM_DATA,  // for imx499
     	{
     		{0x00000499, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
            {0x00000000, 0x00000005, 0x00000002, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
     		{0x00000001, 0x00000020, 0x0000074C, DoCamCalSingleLsc_imx499}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
     		{0x00000001, 0x00000012, 0x0000000E, DoCamCal2AGain_imx499}, //CAMERA_CAM_CAL_DATA_3A_GAIN
     		{0x00000001, 0x0000076E, 0x0000057C, DoCamCalPDAF_imx499},
			{0x00000000, 0x00000FAE, 0x00000550, DoCamCalStereoData} //CAMERA_CAM_CAL_DATA_STEREO_DATA
             /*PDAF Calibration information*/
     	}
     },
     /*zhengjiang.zhu@prize.Camera.Driver  2018/12/15 add for imx499 otp*/
	 //prize fengshangdong modify at 20190130 for 5e9 otp
    {//CALIBRATION_LAYOUT_STEREO_MAIN1_LEGACY:
        0x0000000B, 0x00000001, CAM_CAL_SINGLE_EEPROM_DATA,  //for 5e9
        {
            {0x0000559b, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
            {0x00000000, 0x00000005, 0x00000002, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
            {0x00000001, 0x00000020, 0x0000074C, DoCamCalSingleLsc_5e9}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
            {0x00000001, 0x00000012, 0x0000000E, DoCamCal2AGain_5e9}, //CAMERA_CAM_CAL_DATA_3A_GAIN
    ...
    //prize end
    {//CALIBRATION_LAYOUT_SENSOR_OTP
         0x00000000, 0x00000001, CAM_CAL_SINGLE_EEPROM_DATA,  // for imx476
     	{
     		{0x00000476, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
            {0x00000000, 0x00000005, 0x00000002, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
     		{0x00000001, 0x00000020, 0x0000074C, DoCamCalSingleLsc_imx476}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
     		{0x00000001, 0x00000012, 0x0000000E, DoCamCal2AGain_imx476}, //CAMERA_CAM_CAL_DATA_3A_GAIN
     		{0x00000000, 0x0000076E, 0x0000057C, DoCamCalPDAF},
			{0x00000000, 0x00000FAE, 0x00000550, DoCamCalStereoData} //CAMERA_CAM_CAL_DATA_STEREO_DATA
             /*PDAF Calibration information*/
     	}
     },
...
UINT32 DoCamCalSingleLsc_imx499(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    UINT32 GainValue;
    u16 table_size;
     CAM_CAL_LOG("fsd-----DoCamCalSingleLsc imx499\n");
...
//#endif
    ...
    {
       // if(pCamCalData->sensorID == 0x0499) DoCamCal_Dump(CamcamFID, 0, 6207, pGetSensorCalData);//fengshangdong add 
        ...
        pCamCalData->SingleLsc.LscTable.MtkLcsData.PixId = 0;//hardcode.... need to fix  B 
        CAM_CAL_LOG("fsd-----DoCamCalSingleLsc start_addr is 0x%x\n",start_addr);
        ...
        if(pCamCalData->sensorID == 0x0499)
        {
                table_size = 1868;
        }
        ...
        if(table_size>0)
        {
            ...
            cam_calCfg.u4Offset = (start_addr);//|0xFFFF);
             CAM_CAL_LOG_IF(dumpEnable,"fsd----------------start_addr = 0x%x\n",start_addr);
    ...
    CAM_CAL_LOG_IF(dumpEnable, "CapIspReg =0x%x, 0x%x, 0x%x, 0x%x, 0x%x",
...
UINT32 DoCamCal2AGain_imx499(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	u16 GODEN_rGain = 0,GODEN_bGain = 0,GODEN_gGain = 0;
	
    // define golden vlaue
    //prize fengshangdong disable 
	//u16 GODEN_RGAIN = 473; //R/Gr 
    //u16 GODEN_GGAIN = 512; //Gr/Gb
    //u16 GODEN_BGAIN = 561; //B/Gr
    //end
    ...
    //if(pCamCalData->sensorID == 0x0298) DoCamCal_Dump(CamcamFID, 0, 6207, pGetSensorCalData);//fengshangdong add 
	...
	*/
    ...
    {
            CAM_CAL_LOG("fsd---DoCamCal2AGain enter\n");
            ...
            if(0x1&AWBAFConfig){
                ////AWB////
                cam_calCfg.u4Offset = (start_addr);
				CAM_CAL_LOG_IF(dumpEnable,"fsd--AWB offset=0x%x\n", cam_calCfg.u4Offset);
			...
			if(ioctlerr>0)
            {
				...
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[0] = %d\n", rgbGain[0]);
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[1] = %d\n", rgbGain[1]);
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[2] = %d\n", rgbGain[2]);
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[3] = %d\n", rgbGain[3]);
				CalR  = rgbGain[0];
				CalGr = rgbGain[1];
				CalGb = rgbGain[2];
              ...
              if (CalR!=0 &&
                ...
                CAM_CAL_LOG_IF(dumpEnable,"pCamCalData->Single2A.S2aAwb.rUnitGainu4R = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4R);
				CAM_CAL_LOG_IF(dumpEnable,"pCamCalData->Single2A.S2aAwb.rUnitGainu4G = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4G);
				CAM_CAL_LOG_IF(dumpEnable,"pCamCalData->Single2A.S2aAwb.rUnitGainu4B = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4B);
				
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[4] = %d\n", rgbGain[4]);
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[5] = %d\n", rgbGain[5]);
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[6] = %d\n", rgbGain[6]);
				CAM_CAL_LOG_IF(dumpEnable,"rgbGain[7] = %d\n", rgbGain[7]);
                FacR  = rgbGain[4];
                FacGr = rgbGain[5];
		        FacGb = rgbGain[6];
		        ...
		        if (FacR!=0 &&
		            ...
		            FacB!=0 )
		        {  
		            //prize fengshangdong modify at 20190117 for awb otp
                ...
                #if 0 //not use default golden value 
				pCamCalData->Single2A.S2aAwb.rGoldGainu4R= (u32) ((512.0/GODEN_RGAIN)*512);
				pCamCalData->Single2A.S2aAwb.rGoldGainu4B= (u32) ((512.0/GODEN_BGAIN)*512);
				pCamCalData->Single2A.S2aAwb.rGoldGainu4G= (u32) ((512.0/GODEN_GGAIN)*512);
				#endif
				//prize end

		        }

				CAM_CAL_LOG_IF(dumpEnable,"pCamCalData->Single2A.S2aAwb.rGoldGainu4R = %d\n", pCamCalData->Single2A.S2aAwb.rGoldGainu4R);
				CAM_CAL_LOG_IF(dumpEnable,"pCamCalData->Single2A.S2aAwb.rGoldGainu4G = %d\n", pCamCalData->Single2A.S2aAwb.rGoldGainu4G);
				CAM_CAL_LOG_IF(dumpEnable,"pCamCalData->Single2A.S2aAwb.rGoldGainu4B = %d\n", pCamCalData->Single2A.S2aAwb.rGoldGainu4B);
		        err = CAM_CAL_ERR_NO_ERR;
			...
			if(0x2&AWBAFConfig){
                ...
                cam_calCfg.u4Offset = 0x1A;
                cam_calCfg.u4Length = 2;
                cam_calCfg.pu1Params = (u8 *)&AFInf_value[0];
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                if(ioctlerr>0)
                {
    				err = CAM_CAL_ERR_NO_ERR;
				...
				//prize fengshangdong modify for AF OTP
                AFInf = (AFInf_value[1] + (AFInf_value[0]<<8));
				//prize end 
				CAM_CAL_LOG_IF(dumpEnable,"fsd--AFInf_value[0] = 0x%x\n", AFInf_value[0]);
				CAM_CAL_LOG_IF(dumpEnable,"fsd--AFInf_value[1] = 0x%x\n", AFInf_value[1]);
				CAM_CAL_LOG_IF(dumpEnable,"[AFInf] = %d\n", AFInf);
				
                cam_calCfg.u4Offset = 0x1C;
                cam_calCfg.u4Length = 2;
                cam_calCfg.pu1Params = (u8 *)&AFMacro_value[0];
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                if(ioctlerr>0)
                {
    				err = CAM_CAL_ERR_NO_ERR;
                ...
                //prize fengshangdong modify for AF OTP
				AFMacro = (AFMacro_value[1] + (AFMacro_value[0]<<8));
				//prize end
				CAM_CAL_LOG_IF(dumpEnable,"fsd--AFMacro_value[0] = 0x%x\n", AFMacro_value[0]);
				CAM_CAL_LOG_IF(dumpEnable,"fsd--AFMacro_value[1] = 0x%x\n", AFMacro_value[1]);
...
//prize fengshangdong add at 20190130 for 5e9 otp
UINT32 DoCamCalSingleLsc_5e9(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    stCAM_CAL_INFO_STRUCT  cam_calCfg;
    PCAM_CAL_DATA_STRUCT pCamCalData = (PCAM_CAL_DATA_STRUCT)pGetSensorCalData;
    ...
    UINT32 GainValue;
    u16 table_size;
     CAM_CAL_LOG("fsd-----DoCamCalSingleLsc 5e9\n");
//#ifdef MTK_LOAD_DEBUG
		dumpEnable = 1;
//#else
//		dumpEnable = 0;
//#endif
    ...
    {
        //if(pCamCalData->sensorID == 0x559b) DoCamCal_Dump(CamcamFID, 0, 6207, pGetSensorCalData);//fengshangdong add 
        if(BlockSize!=CAM_CAL_SINGLE_LSC_SIZE)
        {
            CAM_CAL_LOG("BlockSize(%d) is not match (%d)\n", BlockSize, CAM_CAL_SINGLE_LSC_SIZE);
        }
        ...
        if(pCamCalData->sensorID == 0x559b)// for 5e9
        {
                table_size = 1868;
        }
        CAM_CAL_LOG("lsc table_size %d\n", table_size);
...
UINT32 DoCamCalSingleLsc_imx476(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    UINT32 GainValue;
    u16 table_size;
     CAM_CAL_LOG("fsd-----DoCamCalSingleLsc imx476\n");
//#ifdef MTK_LOAD_DEBUG
		dumpEnable = 1;
//#else
//		dumpEnable = 0;
//#endif
    ...
    {
       // if(pCamCalData->sensorID == 0x0476) DoCamCal_Dump(CamcamFID, 0, 6207, pGetSensorCalData);//fengshangdong add 
        if(BlockSize!=CAM_CAL_SINGLE_LSC_SIZE)
        {
            CAM_CAL_LOG("BlockSize(%d) is not match (%d)\n", BlockSize, CAM_CAL_SINGLE_LSC_SIZE);
        }
        ...
        if(pCamCalData->sensorID == 0x0476)
        {
                table_size = 1868;
        }
        CAM_CAL_LOG("lsc table_size %d\n", table_size);
...
UINT32 DoCamCalLayoutCheck(UINT32* pGetSensorCalData)
{
    ...
    if ((g_lastSensorID!=pCamCalData->sensorID))
    {
        ...
        for (i = 0; i< MAX_CALIBRATION_LAYOUT_NUM; i++)
        {
            ...
            //prize fengshangdong modify for otp checkout
...
/***********************************************************************************
 Function : To read PDAF infomation for imx499
************************************************************************************/
UINT32 DoCamCalPDAF_imx499(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize,
	UINT32* pGetSensorCalData)
{
    stCAM_CAL_INFO_STRUCT  cam_calCfg;
    PCAM_CAL_DATA_STRUCT pCamCalData = (PCAM_CAL_DATA_STRUCT)pGetSensorCalData;
    UINT32 ioctlerr;
	CAM_CAL_LOG("fsd----PDAF start_addr\n");
    UINT32 err =  CamCalReturnErr[pCamCalData->Command];
    {
        dumpEnable = 1;
